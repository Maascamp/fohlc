...
public class FifoOffHeapLongCache {
  ...
  /**
   * The load factor we must reach before eviction ends.
   */
  private static final double EVICTION_TARGET = 0.8;
  ...
  private final boolean asyncBookkeeping;
  private BookkeepingThread bookkeeper;

  public static class Builder {

    // cache defaults
    private long builderNumEntries = 1000L;
    private boolean builderAsyncBookkeeping = false;
    private long builderBookkeepingIntervalMillis = 10L;

    public Builder() {}

    public Builder numEntries(long numEntries) {
      builderNumEntries = numEntries;
      return this;
    }

    public Builder asyncBookkeeping(boolean asyncBookkeeping) {
      builderAsyncBookkeeping = asyncBookkeeping;
      return this;
    }

    public Builder bookkeepingIntervalMillis(long bookkeepingIntervalMillis) {
      builderBookkeepingIntervalMillis = bookkeepingIntervalMillis;
      return this;
    }

    public FifoOffHeapLongCache build() {
      return new FifoOffHeapLongCache(
              builderNumEntries,
              builderAsyncBookkeeping,
              builderBookkeepingIntervalMillis);
    }
  }
  ...
  public FifoOffHeapLongCache(
    ...
    this.asyncBookkeeping = asyncBookkeeping;
    if (asyncBookkeeping) {
      bookkeeper = new BookkeepingThread(bookkeepingIntervalMillis, taskBuffer);
      bookkeeper.start();
    }
  }
  ...
  private void evict() {
    if (hasOverflowed()) {
      ...
      } while (getLoadFactor() > EVICTION_TARGET);
    }
  }
  ...
  private class BookkeepingThread extends Thread {
    ...
    public BookkeepingThread(long checkIntervalMillis, Queue<Runnable> taskQueue) {
      super("Bookkeeper");
